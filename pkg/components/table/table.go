...
func (t *Table) Render() app.UI {
    ...
    if len(t.props.Style) > 0 {
        for k, v := range t.props.Style {
            tableElem = tableElem.Style(k, v)
        }
    }
    
    tableElem = tableElem.DataSet("testid", t.props.DataTestID)
    
    // Add caption if provided
    if t.props.Caption != "" {
        tableElem = tableElem.Body(
...
func (t *Table) renderHeader() app.UI {
    ...
    for i, col := range t.props.Columns {
        ...
        // Create header cell with styles
        headerElem := app.Th().
            ID(col.ID).
            Class(headerClass).
            Scope("col")
...
func (t *Table) renderFooter() app.UI {
    ...
    for i, col := range t.props.Columns {
        ...
        // Create footer cell with styles
