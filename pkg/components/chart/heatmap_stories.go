...
func init() {
	// Generate a 20x20 matrix using a sine-wave pattern for visual interest
	rows, cols := 20, 20
	heatmapData := make([][]float64, rows)
	for r := 0; r < rows; r++ {
		heatmapData[r] = make([]float64, cols)
		for l := 0; l < cols; l++ {
			// Creates a "wave" effect: values between 0 and 100
	...
	storybook.Register("Charts", "Heatmap",
		...
		func(controls map[string]*storybook.Control) app.UI {
			return New(nil, // Points are not used for Heatmaps
				WithTitle(controls["Title"].Value.(string)),
				// Custom Heatmap Config Option
				func(c *ChartConfig) {
					// We pass the matrix data through a specialized field in Config
